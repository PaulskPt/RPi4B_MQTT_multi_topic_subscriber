...
def have_ip(printIt:bool = False):
    ...
    if if_dict:
        if isinstance(if_dict, dict):
            ...
            if 2 in if_dict:
                ip = if_dict[2][1]['addr']  # e.g. 192.168.1.140
                if ip != '0.0.0.0':
                    if printIt:
...
def mqtt_callback(topic, msg):
    ...
    try:
        if not use_presto:
            if my_debug:
                #topic = msg.topic
                print(TAG+f"param topic = {topic}, type(topic) = {type(topic)}")
                print(TAG+f"param msg = {msg}, type(msg) = {type(msg)}")
                print(TAG+f"msg.payload = {msg.payload}, type(msg.payload) = {type(msg.payload)}")
        else:
            if my_debug:
        ...
        if use_presto:
            if topic_rcvd.startswith("$SYS"):
                ...
                if broker_topic_in_db():
                    if my_debug:
                        print(TAG+f"topic_rcvd: \"{topic_rcvd}\", payloadStr: \"{payloadStr}\"")
                    return
            else:
                payload = None
                print(TAG+f"Incomplete message \"{msg}\" received, skipping.")
...
def convert_to_dtStr(uxTime):
    ...
    t = time.gmtime(uxTime) # do not use time.localtime(uxTime) because then we end up in Lisbon +1 = Amsterdam time
        ...
        if use_presto:
            ...
            if topic_idx == 0: # sensors/Feath/ambient
                ...
                if use_sense_hat:
                    ...
                    sense.show_message(msg, scroll_speed=0.15, text_colour = txt_colour_soft_white, back_colour=bg)
                    clr_sense_hat()
            elif topic_idx == 1: # lights/Feath/toggle
...
def setup():
    ...
    if use_presto and not my_debug:
    ...
    if use_presto:
        client = mqtt(CLIENT_ID, BROKER, port=PORT) #, recv_buffer=rx_bfr)
